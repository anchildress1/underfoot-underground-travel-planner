...
**All methods RAISE exceptions** instead of returning `False` or `[]`:
...
if not service.store_place_embedding(...):
    logger.error("Storage failed")
```

**Empty `[]` from search now means "no matches found"** not "error occurred":
